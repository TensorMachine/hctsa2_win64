...
elseif (length(tau) == 1) && (length(numGroups) > 1) % vary numGroups
    ...
    % decay to zero
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.2]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c, gof] = BF_fit(numGroupsRange,store(:,1),f);
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.2]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c, gof] = BF_fit(numGroupsRange,store(:,2),f);
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.2]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c, gof] = BF_fit(numGroupsRange,store(:,3),f);
    out.trfexp_a = c.a;
    out.trfexp_b = c.b;
    ...
    % value
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[-0.05 1]);
    f = BF_fittype('a*x+b','options',s);

    r1 = find(store(:,3)>store(1,3)/5);
    if length(r1) > 2
        [~, gof] = BF_fit(numGroupsRange(r1),store(r1,3),f);
    ...
    if length(r2) > 2
        [~, gof] = BF_fit(numGroupsRange(r2),store(r2,3),f);
    ...
    % return the slope
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[0.1 0]);
    f = BF_fittype('a*x+b','options',s);
    c = BF_fit(numGroupsRange,store(:,4),f);
    out.symd_a = c.a;
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.5]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c, gof] = BF_fit(numGroupsRange(r1),store(r1,5),f);
    out.trcovfexp_a = c.a;
    out.trcovfexp_b = c.b;
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.2]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c, gof] = BF_fit(numGroupsRange,store(:,6),f);
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.2]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c, gof] = BF_fit(numGroupsRange,store(:,7),f);
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',[1, -0.2]);
    f = BF_fittype('a*exp(b*x)','options',s);
    [c,gof] = BF_fit(numGroupsRange,store(:,8),f);
