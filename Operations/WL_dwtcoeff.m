...
maxLevelAllowed = BF_wmaxlev(N,wname);
...
if maxLevelAllowed < level
    [c, l] = BF_wavedec(y, maxLevelAllowed, wname);
else
...
for k = 1:level
    if k <= maxLevelAllowed
        d = BF_detcoef(c,l,k); % detail coefficients at level k
        ...
        out.(sprintf('noisestd_l%u',k)) = BF_wnoisest(c,l,k);
    else
