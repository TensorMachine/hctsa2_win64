...
% ------------------------------------------------------------------------------
% d2 writes its results to .c2/.d2/.h2 files and prints NOTHING to stdout, so
% keep stderr: it is the only text there is, and the check below tests it.
[~, res] = BF_RunTisean(sprintf('d2 -d%u -M1,%u -t%u %s',tau,maxm,theilerWin,filePath),true);
...
% Success is really "did the output files appear", not "was anything printed".
producedFiles = exist([filePath '.c2'],'file') || exist([filePath '.d2'],'file');
if (isempty(res) && ~producedFiles) || ~isempty(regexp(res,'command not found','once'))
...
[~, res] = BF_RunTisean(sprintf('c2g %s.c2',filePath));
% output is in res -- check it
s = textscan(res,'%[^\n]'); s = s{1};
 s = strtrim(regexprep(s,'\r',''));   % Windows TISEAN emits CRLF
% The "writing to stdout" line is stderr output, which BF_RunTisean discards,
% and Windows buffering could place it after the data anyway. Keep every line;
% SUB_readTISEANout locates its blocks by the '#m=' / '#dim=' markers, which are
% genuine stdout content.
...
try
    ...
    error('NL_TISEAN_d2 could not parse the c2g output.\n%s',parseErr.message)
end
...
% ------------------------------------------------------------------------------
    function dimdat = SUB_readTISEANout(s,maxm,blocker,nc)
        ...
        if length(w)~=maxm
            % Say what was actually seen. The caller used to rewrite this as
            % "probably some Inf and NAN values", which is a guess and was
            % usually wrong -- the real cause is a marker count mismatch.
            preview = '';
            for kk = 1:min(8,length(s))
                preview = [preview sprintf('\n    |%s|',s{kk})];
            end
            error(['TISEAN output parse failed: found %u ''%s'' markers, ' ...
                   'expected %u (maxm). First lines of the output:%s'], ...
                   length(w),blocker,maxm,preview);
        end
