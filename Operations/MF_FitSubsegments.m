...
switch model
    ...
    case 'ss'
        ...
        fpes = zeros(numPred,1);
        for i = 1:numPred
            try  m = BF_n4sid(y(r(i,1):r(i,2)),order);
            catch
    ...
    case 'arma'
        ...
        for i = 1:numPred
            try
                m = BF_armax(y(r(i,1):r(i,2)),order);
            catch emsg
        ...
        % order is a 2-vector [na, nc] here, so the AR loop must use order(1).
        % `1:order` errors in modern MATLAB ("Colon operands must be real
        % scalars"); older releases and Octave silently took the first element,
        % which happened to be right for the AR loop and wrong for the MA one.
