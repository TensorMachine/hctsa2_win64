...
function [c,gof] = f_fix_exp(r,dataVector,startPoint,addOffset)
    ...
    s = BF_fitoptions('Method','NonlinearLeastSquares','StartPoint',startPoint);
    if addOffset
...
function out = assignExpStats(out,c,gof,fieldName)
    ...
    % Does this fit have a third coefficient? Test the struct and the cfit
    % cases separately: `||` short-circuits only when the LEFT side is true,
    % so a BF_fit struct without a 'c' field still fell through to
    % coeffnames(), which is a Curve Fitting Toolbox function.
    if isstruct(c)
        hasC = isfield(c,'c');
    else
        hasC = ismember('c',coeffnames(c));   % cfit object
    end
    if hasC
