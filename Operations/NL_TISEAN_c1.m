...
% run c1 code
tic
% c1 writes its result to a .c1 file and prints nothing to stdout, so keep
% stderr -- it is the only text, and the isempty(res) check below relies on it.
[~, res] = BF_RunTisean(sprintf('c1 -d%u -m%u -M%u -t%u -n%u -o %s.c1 %s',...
            tau,mmm(1),mmm(2),tsep,Nref,filePath,filePath),true);
delete(filePath) % remove the temporary data file

if isempty(res) && ~exist([filePath '.c1'],'file')
    % Empty output AND no file: it really did fail. Empty output with the file
    % present just means c1 said nothing, which is normal on some builds.
...
[~, res] = BF_RunTisean(sprintf('c2d -a2 %s.c1',filePath));
