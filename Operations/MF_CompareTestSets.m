...
switch theModel
    case 'ar' % fit an ar model of specified order
        if strcmp(ord,'best')
            ...
            try
                [west, Aest, Cest, SBC, FPE, th] = ARFIT_arfit(y, 1, 10, 'sbc', 'zero');
            catch
...
for i = 1:numPred
    ...
    yp = BF_predict(m, yTest, steps); % across test set using model, m,
                                   % fitted to entire data set

    %     e = BF_pe(m, yTest); % prediction errors -- exactly the same as returning
    ...
    meandiffs(i) = abs(mean(yp) - mean(yTest));
    stdrats(i) = std(yp)/std(yTest);
